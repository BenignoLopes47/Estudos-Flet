...
# Deteta a pasta exata onde este ficheiro .py está guardado
DB_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(DB_DIR, "lista_compras.json")
...
async def main(page: ft.Page):
    ...
    # Grava diretamente no ficheiro json da diretoria corrente
    async def save_data():
        data = []
        for item in items_list.controls:
            data.append({"text": item.text, "completed": item.completed})
        
        with open(DB_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)

    # Carrega do ficheiro json se ele existir na mesma diretoria
    async def load_data():
            ...
            with open(DB_PATH, "r", encoding="utf-8") as f:
                ...
                for item in data:
                    items_list.controls.append(
                        ShoppingItem(item["text"], item["completed"], item_updated, item_deleted)
                    )
            page.update()
